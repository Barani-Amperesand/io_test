...
class RegisterDecoder:
    ...
    def __init__(self, mc_file_path: str, lc_file_path: str):
        ...
        """
        print(f"INFO: Initializing decoder with MC map: '{os.path.basename(mc_file_path)}'", file=sys.stderr)
        print(f"INFO: Initializing decoder with LC map: '{os.path.basename(lc_file_path)}'", file=sys.stderr)
    ...
    def decode(self, address: Union[int, str], value: Union[int, str], do_print: bool = False) -> Optional[List[Dict[str, Any]]]:
        """
        ...
        try:
            ...
            print(f"Warning: Invalid address or value format. Skipping.", file=sys.stderr)
            return None

        lookup_addr, block_info, map_type = self._find_address_context(address_int)
        
        if not lookup_addr:
            if do_print: print(f"Warning: Address {hex(address_int).upper()} not found in memory map. Skipping.", file=sys.stderr)
            return None
            
        active_map = self.mc_map if map_type == 'MC' else self.lc_map
        if lookup_addr not in active_map:
            if do_print: print(f"Warning: Address {hex(address_int).upper()} decoded to lookup address {lookup_addr}, which is not in the {map_type} map. Skipping.", file=sys.stderr)
            return None
        ...
        for field_data in fields:
            label, slice_str, data_format = field_data['label'], field_data['slice'], field_data.get('format', 'decimal')
            try:
        ...
        if do_print:
            self._print_decoded_output(hex(address_int), value_int, block_info, fields[0]['description'], decoded_fields)

        return decoded_fields

    def _print_decoded_output(self, address_str: str, value_int: int, block_info: dict, description: str, decoded_fields: list):
        ...
        for field in decoded_fields:
            ...
            display_value = self._format_display_value(field['interpreted_value'], field['format'])
            print(f"{prefixed_label:<35} {field['slice']:<10} {field['hex_value']:<15} {display_value:<17}")
        print("-" * 80 + "\n")
    ...
    def _build_mc_memory_map() -> List[Dict[str, Any]]:
        mem_map, block_size = [{'name': 'Master RegIface (conf)', 'start_addr': 0xA0000000, 'end_addr': 0xA00001FF, 'map_to_use': 'MC'},
                               {'name': 'Master RegIface (stat)', 'start_addr': 0xA0008000, 'end_addr': 0xA00081FF, 'map_to_use': 'MC'}], 0x200
        endpoints = [{'name': 'LVC', 'prefix': 'lv_', 'offset': 0x000}, {'name': 'MVC1', 'prefix': 'mv1_', 'offset': 0x200}, {'name': 'MVC2', 'prefix': 'mv2_', 'offset': 0x400}]
        for link_num in range(1, 13):
            tx_base, rx_base = 0xA0000000 + (link_num * 0x800), 0xA0008000 + (link_num * 0x800)
            for ep in endpoints:
                mem_map.append({'name': f"Link-{link_num} - {ep['name']}TX", 'start_addr': tx_base + ep['offset'], 'end_addr': tx_base + ep['offset'] + block_size - 1, 'map_to_use': 'LC', 'prefix': ep['prefix']})
                mem_map.append({'name': f"Link-{link_num} - {ep['name']}RX", 'start_addr': rx_base + ep['offset'], 'end_addr': rx_base + ep['offset'] + block_size - 1, 'map_to_use': 'LC', 'prefix': ep['prefix']})
        return mem_map
    ...
    def _load_register_map(excel_path: str) -> Dict[str, List[Dict[str, str]]]:
        ...
        try:
            ...
            if not all(col in df.columns for col in REQUIRED_COLUMNS):
                print(f"Error: Excel file '{excel_path}' is missing {set(REQUIRED_COLUMNS) - set(df.columns)}", file=sys.stderr); sys.exit(1)
