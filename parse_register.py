...
def load_register_map(excel_path: str) -> Dict[str, List[Dict[str, str]]]:
    """
    Parses a register map from an Excel file, handling RegSize to clone
    register definitions and adding 1-based index suffixes to their labels.
    """
    register_map = {}
...
def process_and_print_register(address_str: str, value_str: str, mc_map: dict, lc_map: dict):
    ...
    print(f"{'Label':<35} {'Slice':<10} {'Value (Hex)':<15} {'Value (Dec)':<17}")
    ...
    for field in fields:
        ...
        try:
            ...
            print(f"{prefixed_label:<35} {slice_str:<10} {'0x' + hex(field_value)[2:].upper():<15} {field_value:<17}")
...
def main():
    ...
    if is_file_mode:
        # UPDATED REGEX: Matches one or more spaces OR colons as a separator.
